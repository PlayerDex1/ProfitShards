...
export function ActivityFeedAlt() {
  ...
  const loadFeed = async () => {
    ...
    console.log('🔄 ActivityFeedAlt: Carregando dados forçados...');
    
    try {
      // Tentar API forçada primeiro
      const response = await fetch('/api/feed/force-real-data', {
        method: 'GET',
        credentials: 'include',
      });
      
      const result = await response.json();
      console.log('📊 Resultado API forçada:', result);
      
      if (result.success && result.runs && result.runs.length > 0) {
        setRuns(result.runs);
        console.log(`✅ ${result.runs.length} runs carregadas com sucesso`);
        return;
      }
      
      // Se API forçada falhar, tentar API normal
      console.log('⚠️ API forçada sem dados, tentando API normal...');
      const fallbackResponse = await fetch('/api/feed/recent-runs', {
        method: 'GET', 
        credentials: 'include',
      });
      
      const fallbackResult = await fallbackResponse.json();
      console.log('📊 Resultado API normal:', fallbackResult);
      
      if (fallbackResult.success) {
        setRuns(fallbackResult.runs || []);
      } else {
        setError(fallbackResult.error || 'Erro ao carregar feed');
      }
      
    } catch (error) {
      console.error('❌ Erro ao carregar feed alternativo:', error);
  ...
  useEffect(() => {
    ...
    // Auto-refresh a cada 1 minuto (mais frequente)
      ...
      <Card className="mt-8 bg-gradient-to-br from-primary/5 to-blue-600/10 border border-primary/20">
        ...
        <CardContent>
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-xs mt-1">Runs aparecerão aqui quando dados forem salvos</p>
          </div>
        </CardContent>
      </Card>
    );
    ...
    <Card className="mt-8 bg-gradient-to-br from-primary/5 to-blue-600/10 border border-primary/20 hover:shadow-lg hover:scale-[1.01] transition-all duration-300">
      ...
      <CardContent>
        ...
        {runs.length > 9 && (
          <div className="text-center mt-4 pt-4 border-t border-border/50">
            <p className="text-xs text-muted-foreground">
              Mostrando 9 de {runs.length} atividades encontradas
            </p>
          </div>
        )}
        ...
        <div className="text-center mt-4 pt-2 border-t border-primary/10">
          <p className="text-xs text-muted-foreground">
            🔧 Feed alternativo • Auto-refresh 1 min • Dados diretos do D1
          </p>
        </div>
      </CardContent>
    </Card>
