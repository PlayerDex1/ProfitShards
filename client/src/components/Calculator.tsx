...
export const Calculator = memo(function Calculator({ formData, results, onUpdateFormData, onSaveToHistory }: CalculatorProps) {
	...
	// Hook para gerenciar aceleramentos de equipamentos
	const {
				...
				<div className="space-y-4">
					...
					{EQUIPMENT_CONFIGS.map((config) => (
						<EquipmentCard
							...
							config={config}
							formData={formData}
							equipmentData={equipmentAccelerations[config.key]}
							newAcceleration={newAcceleration}
							onToggleExpanded={toggleExpanded}
							onAddAcceleration={addAcceleration}
							onUpdateNewAcceleration={(field, value) => 
								setNewAcceleration(prev => ({ ...prev, [field]: value }))
							}
