...
export function ProfitSensitivityChart({ currentTokenPrice, results, labelY = 'Lucro ($)' }: ProfitSensitivityChartProps) {
  const data = useMemo(() => {
    ...
    // generate prices from 50% to 150% of current, 11 points
    for (let i = 0; i <= 10; i++) {
      const factor = 0.5 + (i * 0.1);
      const price = Number((currentTokenPrice * factor).toFixed(4));
      const delta = price - currentTokenPrice;
      const profit = results.finalProfit + delta * totalTokens; // linear sensitivity to token price
      ...
      <LineChart data={data} margin={{ top: 10, right: 20, left: 10, bottom: 10 }}>
        ...
        <Tooltip 
          formatter={(value: number, name: string, _props: any) => {
            return [ `$${Number(value).toFixed(2)}`, name === 'profit' ? 'Lucro' : 'Preço' ];
          }} 
