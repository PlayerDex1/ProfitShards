...
// POST - Salvar nova run do MapPlanner
export async function onRequestPost({ env, request }: { env: Env; request: Request }) {
  try {
    console.log('💾 FEED RUNS: Salvando nova run do MapPlanner...');
    ...
    const runData: NewRunData = await request.json();
    console.log('📊 Dados recebidos do MapPlanner:', runData);

    // Validação básica
    if (!runData.mapSize || !runData.tokensDropped) {
      return returnErrorResponse('Missing required fields', 400);
    }

    // Processar dados da run
    const timestamp = runData.timestamp || Date.now();
    const runId = `run-${timestamp}-${Math.random().toString(36).substr(2, 9)}`;

    // Mapear tamanho do mapa
    const mapNames: Record<string, string> = {
      'small': 'Small Map',
      'medium': 'Medium Map', 
      'large': 'Large Map',
    ...
    const mapName = mapNames[runData.mapSize] || runData.mapSize;

    // Inserir na tabela user_map_drops (tabela principal que o feed lê)
    try {
      await env.DB.prepare(`
        INSERT INTO user_map_drops (
          id, user_id, map_name, tokens_earned, drop_data, created_at
        ) VALUES (?, ?, ?, ?, ?, ?)
      `).bind(
        runId,
        'feed-user', // User ID genérico para feed
        runData.mapSize, // Usar formato original (small/medium/large/xlarge)
        runData.tokensDropped,
        JSON.stringify({
          luck: runData.luck || 0,
          level: runData.level || 'IV',
          tier: runData.tier || 'I', 
          charge: runData.charge || 4,
          efficiency: runData.luck ? (runData.tokensDropped / runData.luck * 1000) : 0,
          playerEmail: runData.userEmail || 'anonymous'
        }),
        timestamp
      ).run();

      console.log(`✅ Run salva no feed: ${mapName} - ${runData.tokensDropped} tokens`);

      return new Response(JSON.stringify({
        success: true,
        message: 'Run saved to feed successfully',
        data: {
          id: runId,
          mapName: mapName,
          tokens: runData.tokensDropped,
      ...
      return returnErrorResponse('Database error', 500, dbError.message);
    }

  } catch (error) {
    console.error('❌ Erro geral salvando run:', error);
    return returnErrorResponse('Internal server error', 500, error.message);
  }
}
