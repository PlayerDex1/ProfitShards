...
export async function onRequestGet({ env, request }: { env: Env; request: Request }) {
  try {
    ...
    const row = await (env as any).DB.prepare(`SELECT u.id, u.email FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.session_id = ? AND s.expires_at > ?`).bind(sid, Date.now()).first<{ id: string; email: string }>();
